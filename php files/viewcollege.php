...
if (isset($_POST['submit']))
       {
        ...
        $collegetime = $_FILES['collegetime']['name'];
        $tmp_name = $_FILES['collegetime']['tmp_name'];
        $uploadDir = "upload/";
        move_uploaded_file($tmp_name,$uploadDir.$collegetime);

        $insertquery = "INSERT into ctimetable(collegetime) values ('$collegetime')" ;

        $res = mysqli_query($con,$insertquery);

        if ($res && !(empty($collegetime)))
        {
            ?>
           ...
           <script>
            ...
            <script>
                ...
                location.replace("collegetime.php");
            </script>
            <?php
        }
...
<?php 
    ...
    $query = " SELECT * from ctimetable order by id desc" ;   
...
<div class="main-div">
    <h1 class="mang">College Time Table</h1>
                ...
                while ($rows = mysqli_fetch_assoc($result))
                    {
                        ...
                        <tr>
                            ...
                            <td><a href="upload/<?php echo $rows['collegetime'];?>" class="btn btn-primary"><?php echo $rows['collegetime']?></a></td>
                            <td><a download="<?php echo $rows['collegetime'];?>" href="upload/<?php echo $rows['collegetime']?>" class="btn btn-success">Download</a></td>
                            <td><?php echo $rows['up_time']?></td>
                            <td><a href="http://localhost/project/php files/collegetimedelete.php?Aid=<?php echo $rows['id'];?>" class="btn btn-primary">Delete</a></td>
                <?php
                    } 
                ?>
