...
if (isset($_POST['submit']))
       {
        ...
        $examtime = $_FILES['examtime']['name'];
        $tmp_name = $_FILES['examtime']['tmp_name'];
        $uploadDir = "upload/";
        move_uploaded_file($tmp_name,$uploadDir.$examtime);

        $insertquery = "INSERT into etimetable(examtime) values ('$examtime')" ;

        $res = mysqli_query($con,$insertquery);

        if ($res && !(empty($examtime)))
        {
            ?>
...
<?php 
    ...
    $query = " SELECT * from etimetable order by id desc" ;   
...
<div class="main-div">
    <h1 class="mang">Exam Time Table</h1>
                ...
                while ($rows = mysqli_fetch_assoc($result))
                    {
                        ?>
                        <tr>
                            <td><a href="upload/<?php echo $rows['examtime'];?>" class="btn btn-primary"><?php echo $rows['examtime']?></a></td>
                            <td><a download="<?php echo $rows['examtime'];?>" href="upload/<?php echo $rows['examtime']?>" class="btn btn-success">Download</a></td>
                            <td><?php echo $rows['up_time']?></td>
                            <td><a href="http://localhost/project/php files/examtimedelete.php?Aid=<?php echo $rows['id'];?>" class="btn btn-primary">Delete</a></td>
                <?php
                    } 
                ?>
